...
6. `--mode draft` to judge. `--mode final` to upload (dots picked for you).
...
2. Tool picks 4500 dots for sharp letters. Wait ~30s per 5s of video.
3. Upload the `.final.mp4`.

## Melt words into a flower

1. After a title `rest`, add `grow(Bloom(scale=0.44, jitter=0.9), 2.2)`.
